 ...
 */
class ideaCustomer extends MY_Controller {
    ...
    public function index() {
        ...
        $tpl = array(
            'breadcrumb' => array(
                base_url() => 'Home',
                base_url('ideaCustomer') => 'Ý kiến khách hàng'),
        );
        $item = $this->input->get();
        ...
        $total_records = 0;
        $data['list_data'] = $this->mcomment->listAllIdeaComment($item, $total_records, $offset, $page_config['per_page']);
        ...
        $tpl["main_content"] = $this->load->view('ideaCustomer/index', $data, TRUE);
    ...
    public function add() {
        ...
        $tpl = array(
            'breadcrumb' => array(
                base_url() => 'Home',
                base_url('ideaCustomer') => 'Khách hàng hỏi - saxa trả lời',
                base_url('ideaCustomer/add') => 'Tạo mới'),
        );
        ...
        $tpl["main_content"] = $this->load->view('ideaCustomer/new', $data, TRUE);
        $this->load->view(TEMPLATE, $tpl);
    }
    ...
    private function _validateForm(&$data, &$errors) {
        ...
        // Set rules:
        $this->form_validation->set_rules("name", 'Nhập tên khách hàng', "required|trim|xss_clean|max_length[255]");
        $this->form_validation->set_rules("idea", 'Nhập ý kiến', "required|trim|max_length[255]|");
