 ...
 * @date 2015/06/22
 */
class mnews_saxa_everyday extends MY_Model
{
    ...
    public function search($params = '',&$total, $offset = 0, $limit = 0)
    {
        ...
        $sql = "SELECT
                ...
                FROM
                        " . $this->_tbl_news_saxa_everyday . " AS n
                INNER JOIN " . $this->_tbl_saxa_everyday . " AS cn
    ...
    public function save($params)
    {
        ...
        if (!empty($params['news_id'])) {
            ...
            return $this->db->update($this->_tbl_news_saxa_everyday, $data);
        } else {
            $data['create_user'] = $this->session->userdata('ses_user_id');
            $data['create_date'] = date('Y-m-d H:i:s');
            return $this->db->insert($this->_tbl_news_saxa_everyday, $data);
        }
    }
    ...
    public function del($newsId)
    {
        ...
        return $this->db->update($this->_tbl_news_saxa_everyday, array('del_flg' => 1));
    }
    ...
    public function checkExistTitle($title)
    {
        $this->db->select('id')
                ->where('title', $title);
        $query = $this->db->get($this->_tbl_news_saxa_everyday);
        if ($query->num_rows() > 0) {
            return TRUE;
        } else {
            return FALSE;
