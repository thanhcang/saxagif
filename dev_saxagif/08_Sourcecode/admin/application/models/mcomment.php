...
class Mcomment extends MY_Model {
     ...
     * @param type $param
     */
    public function addIdea($param) {
        $data = array(
            'idea' => htmlspecialchars($param['idea']),
            'name' => htmlspecialchars($param['name']),
            'language_type' => !empty($param['language_type']) ? $param['language_type'] : 1,
            'logo' => $param['logo'],
        );

        $this->db->insert('d_idea_customer', $data);
    }
     ...
     * @return boolean
     */
    public function listAllIdeaComment($param, &$total, $offset = 0, $limit = 0) {
        $arr_where = array();
        $sql = "SELECT
                *
                FROM d_idea_customer ";
